...
export const SignInPage: React.FC = () => {
  ...
  const onSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (!hasError(newError)) {
      const response = await postWithoutLoading<{ jwt: string }>('https://mangosteen2.hunger-valley.com/api/v1/session', data)
        .catch(onSubmitError)
  ...
  const sendSmsCode = async () => {
    ...
    const response = await postWithLoading('https://mangosteen2.hunger-valley.com/api/v1/validation_codes',
      { email: data.email }
    )
    return response
  }
