...
export const RankChart: React.FC<Props> = (props) => {
    ...
    const max = items?.reduce((pre, item) => Math.max(pre, parseFloat(item.value.toString())), 0) ?? 0
            ...
            <div >
                {items?.map((item, index) => <div key={item.name} b-1 b-solid b-red grid grid-cols='[48px_1fr_1fr]'
                    ...
                    <div row-start-1 col-start-2 row-end-2 col-end-3 self-end >
                        {item.name}-{`${(parseFloat(item.value.toString()) / total * 100).toFixed(0)}%`}</div>
        ...
        <div ref={div} className={className}>
            {
                items?.[0]
                    ? renderItems()
                    : <div text-center color="[var(--color-pink)]">暂无数据</div>
            }
